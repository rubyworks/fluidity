...
provides a flexible grammer system which can be used by requiring any of the
prime grammer methods that extend BasicObject. The most common is probably `should`.

  require 'fluidity/should'

  10.should.be.kind_of(Integer)

But if you are acustom to MiniTest's spec methods, you might prefer `must`.

  require 'fluidity/must'

  10.must.be.kind_of(Integer)

Aslo provided is `assert` for those techy aficionados.

  require 'fluidity/assert'

  10.assert.kind_of(Integer)

So what so fludi about all this. Well, Fluidity allows the developer quite
